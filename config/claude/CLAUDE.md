...
This file provides enterprise-wide guidance to Claude Code (claude.ai/code) across all SimpleMotion repositories. It is loaded automatically for every project via `~/.claude/CLAUDE.md`.
...
SimpleMotion-9000-0000-00-Govern            9000-Govern/         9000-Govern
(container for repos)                       (local mirror)       (org home directory)
```

**Each organization contains:**
- `XXXX-Name` — Home directory repo (from `990002-SG-99-SimpleMotion-Folder` template)
- `.github` / `.github-private` — GitHub config submodules (from 9900-0000-00-Templates)

**Plugins** are centralised in monorepos (`simplemotion/sm-plugins`, `simpleglobal/sg-plugins`), not in individual org repos.

**Submodules:** `.claude`, `.github`, `.github-private`, `.simplemotion` — all sourced from 9900-0000-00-Templates.

### Submodule Version Policy

**RULE: All `.claude` submodules MUST track the latest version of `.claude-executive` in SimpleMotion-9900-0000-00-Templates.**
...
**RULE: Symlinks from `~/` must use `SimpleMotion/.userconfig/{file}` as target (no `../` prefix).** The `../` form breaks because `~/..` resolves to `/Users/`, not `~/`.

```bash
# CORRECT
ln -s SimpleMotion/.userconfig/.gitconfig ~/.gitconfig
# WRONG
ln -s ../SimpleMotion/.userconfig/.gitconfig ~/.gitconfig
```
...
All SimpleMotion identifiers follow a consistent `XXXX-XXXX` hyphenated block structure.

#### Organisations and Repositories

| Entity Type | Pattern | Example |
|-------------|---------|---------|
| First-Tier Org | `SimpleMotion-XX00-0000-00-Name` | `SimpleMotion-9000-0000-00-Govern` |
| Second-Tier Org | `SimpleMotion-XXYY-0000-00-Name` | `SimpleMotion-9200-0000-00-Employees` |
| Simple Sub-Org | `SimpleMotion-0X00-0000-SM-Name` | `SimpleMotion-0300-0000-SM-Entertainment` |
| Project Org | `SimpleMotion-YYYY-0000-00-Projects` | `SimpleMotion-2026-0000-00-Projects` |
| Local Folder | `XXXX-Name/` | `9000-Govern/` |
| Org Home Repo | `XXXX-Name` | `9000-Govern` |
| Plugin (monorepo subdir) | `sm-{name}` | `sm-govern` (in `sm-plugins`) |
| Default Template | `990001-SG-99-SimpleMotion-Default` | Standard repo template |
| Folder Template | `990002-SG-99-SimpleMotion-Folder` | Home directory template |
| Employee Home | `9200-XXXX-ST-SimpleMotion-Name` | `9200-0001-ST-SimpleMotion-Greg-Gowans` |
...
Template inheritance: Source templates in 9900-0000-00-Templates (`.claude-executive`, `.github`, `.github-private`) flow into `990001` (Default) and `990002` (Folder). Those templates create all org home repos. Plugins live in monorepos (`simplemotion/sm-plugins`, `simpleglobal/sg-plugins`).

**Update flow:** Update source template → pull into 990001/990002 → propagate submodule updates across all repos.

## Plugin Architecture

Every tier-1 org has a plugin (`sm-{name}`) providing Claude Code commands for that domain. All plugins live as subdirectories in a **monorepo**: `simplemotion/sm-plugins` (SM) and `simpleglobal/sg-plugins` (SG). Tier-2 orgs do not have plugins — they inherit their parent tier-1 plugin.

### Monorepo Structure

```
sm-plugins/                           # simplemotion/sm-plugins
├── .claude-plugin/
│   └── marketplace.json              # Marketplace manifest (inline, not separate repo)
├── sm-simple/                        # One subdirectory per plugin
...
```json
{
  "name": "simplemotion",
  "plugins": [
    { "name": "sm-govern", "source": "./sm-govern" }
  ]
}
```
...
**Naming:** `sm-{name}` matching the tier-1 org short name (e.g. `sm-govern` for 9000-Govern).
...
```bash
# Clone the monorepo
git clone https://github.com/simplemotion/sm-plugins.git
cd sm-plugins
...
|--------|------------|---------|
| Email, Calendar, Contacts, Teams | M365 (`/sm-mcp-m365`) | All Microsoft 365 operations |
| Invoices, Quotes, POs, Contacts | Xero (`/sm-mcp-xero`) | All Xero operations |

**RULE: Authenticate BEFORE any M365/Xero operation:** `/sm-mcp --auth` (or individually `/sm-mcp-m365 --auth`, `/sm-mcp-xero --auth`). Tokens expire ~1 hour; re-auth on "TokenExpired" or 401 errors.

**Token storage:** macOS Keychain using `{Profile}-{Service}-{Type}` format (profiles: SP, SM). Diagnostics: `/sm-mcp-m365 --status`, `/sm-mcp-xero --status`.
...
| `~/.claude/.mcp.json` | User-level MCP servers (sm-mcp-xero, sm-mcp-m365) |
| `~/.claude/plugins/known_marketplaces.json` | Registered plugin marketplaces |

All symlinked via `ln -sf ../SimpleMotion/.userconfig/.claude/{file}` (relative from `~/.claude/`). Run `/sm-manage:sm-setup` to create or refresh these symlinks.
...
├── .simplemotion/         # Submodule → .simplemotion (org framework config)
...
**RULE: All new repos MUST be created from `990001-SG-99-SimpleMotion-Default` template. All org home repos MUST use `990002-SG-99-SimpleMotion-Folder` template. All repos are internal visibility.**

```bash
# Standard repo
gh repo create <org>/<repo-name> --template SimpleMotion-9900-0000-00-Templates/990001-SG-99-SimpleMotion-Default --internal --description "Description"

# Org home directory repo
gh repo create <org>/<folder-name> --template SimpleMotion-9900-0000-00-Templates/990002-SG-99-SimpleMotion-Folder --internal --description "SimpleMotion - Engineered for Architecture, Entertainment and Industry."
```
...
- Standard submodules: `.claude`, `.github`, `.github-private`, `.simplemotion`
...
| Name | Email |
|------|-------|
| Greg | greg.gowans@simplemotion.com |
| Joy | joy.yeoh@simplemotion.com |
| Kenji | kenji.oates@simplemotion.com |
| Ryan | ryan.sturgeon@simplemotion.com |
...
Email: greg.gowans@simplemotion.com
