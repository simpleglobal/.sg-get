...
Full design rationale and implementation details for the SimpleMotion executive onboarding bootstrap.
...
The script asks for one thing: the user's `@simplemotion.com` email. Everything else is inferred:

- **Full name**: title-case the email prefix, split on `.` (e.g. `joy.yeoh` → `Joy Yeoh`)
- **GitHub username**: title-case, hyphenated, `-SM` suffix (e.g. `Joy-Yeoh-SM`)
...
The `.mcp.json` template has `sm-mcp-xero` and `sm-mcp-m365` set to `disabled: true` with empty command paths. On a fresh machine, the MCP Rust binaries don't exist yet. They are compiled and configured later via `/sm-mcp --init` once the user is working in Claude Code.

All 11 official Claude AI connectors (`claude_ai_Asana`, etc.) are also explicitly disabled per enterprise policy — SimpleMotion uses custom MCP servers only.
...
### Rust in ~/SimpleMotion/.cargo/

Rust is installed with `CARGO_HOME=~/SimpleMotion/.cargo` and `RUSTUP_HOME=~/SimpleMotion/.rustup`, keeping all tooling inside the project tree. The `--no-modify-path` flag prevents rustup from editing shell configs (`.zshenv` already handles PATH).
...
| 7 | Clone employee repo | Search Employees org, clone to `~/SimpleMotion` (or mkdir fallback) |
| 8 | Install Rust | rustup with custom CARGO_HOME/RUSTUP_HOME, `--no-modify-path` |
| 9 | Deploy config | Copy templates to `~/SimpleMotion/.userconfig/`, sed placeholders |
...
```
~/.gitconfig  → SimpleMotion/.userconfig/.gitconfig
~/.zshenv     → SimpleMotion/.userconfig/.zshenv
~/.zshrc      → SimpleMotion/.userconfig/.zshrc
```
...
```
~/.claude/CLAUDE.md       → ../SimpleMotion/.userconfig/.claude/CLAUDE.md
~/.claude/settings.json   → ../SimpleMotion/.userconfig/.claude/settings.json
~/.claude/.mcp.json       → ../SimpleMotion/.userconfig/.claude/.mcp.json
```

Target uses `../` because the symlink lives inside `~/.claude/`, so `../` goes up to `~/`.

### Plugins (`~/.claude/plugins/` → `../../SimpleMotion/...`)

```
~/.claude/plugins/known_marketplaces.json → ../../SimpleMotion/.userconfig/.claude/plugins/known_marketplaces.json
~/.claude/plugins/blocklist.json          → ../../SimpleMotion/.userconfig/.claude/plugins/blocklist.json
```
...
| Secret | Value |
|--------|-------|
| `AZURE_TENANT_ID` | SimpleMotion Azure AD tenant ID |
| `AZURE_CLIENT_ID` | Azure AD app registration client ID |
| `AZURE_CLIENT_SECRET` | Azure AD app registration client secret |

The Azure AD app requires `Mail.Send` application permission for `greg.gowans@simplemotion.com`.

### Email content

- Subject: "Welcome to SimpleMotion — Setup Complete"
- From: `greg.gowans@simplemotion.com`
...
- [ ] `ls -la ~/.zshrc` shows symlink → `SimpleMotion/.userconfig/.zshrc`
- [ ] `ls -la ~/.zshenv` shows symlink → `SimpleMotion/.userconfig/.zshenv`
- [ ] `ls -la ~/.gitconfig` shows symlink → `SimpleMotion/.userconfig/.gitconfig`
- [ ] `ls -la ~/.claude/CLAUDE.md` shows symlink → `../SimpleMotion/.userconfig/.claude/CLAUDE.md`
- [ ] `ls -la ~/.claude/settings.json` shows symlink → `../SimpleMotion/.userconfig/.claude/settings.json`
...
- [ ] Opening a new terminal and typing `sm` launches Claude Code in `~/SimpleMotion`
- [ ] Running `/sm-orgs --sync-folders` in Claude Code pulls the org structure
